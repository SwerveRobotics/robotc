...
// Format a I2C message that requests data from a certain I2C address and register,
// together with a certain amount of outbound payload data
#define FormatI2CPrim(req, addr, ibOffset, cbPayload)                               \
    ...
    i2cOffsetOf(req) = (ibOffset);              /* offset/reg# */                   \
    }
...
#define i2cSend(fSuccess, link, req, cbReply)                                       \
    ...
    if (fSuccess)                                                                   \
        ...
        if (!fSuccess)                                                              \
            {                                                                       \
            /* TRACE(("i2cSend: bus fail after send: %d", link)); */                \
            }                                                                       \
        }                                                                           \
    else                                                                            \
        {                                                                           \
        /* TRACE(("i2cSend: bus fail before send: %d", link)); */                   \
        }                                                                           \
    }
