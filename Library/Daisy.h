...
void CheckMotorForFault(OUT BOOL& fSuccess, MOTORCONTROLLER& controller)
	{
	fSuccess = controller.fActive;
	if (fSuccess)
	    {
        FormatI2CReq(reqDaisy, controller.i2cAddr, 0x44);
        fSuccess = i2cSendReceiveDaisy(controller.link, reqDaisy, repDaisy, 4);
        if (fSuccess)
            {
            int mode0  = repDaisy.rgb[IbI2CPayload(0)];
            int power0 = repDaisy.rgb[IbI2CPayload(1)];
            int power1 = repDaisy.rgb[IbI2CPayload(2)];
            int mode1  = repDaisy.rgb[IbI2CPayload(3)];
            //
            // We don't currently do anything with this info other than check the
            // error state of the motor controller.
            //
            if ((mode0 & MOTORMODE_ERROR))
                {
                TRACE(("error: motor0 on mtrcnt(%d) faulted", controller.imotorctlr+1));
                Beep();
                fSuccess = false;
                }
            if ((mode1 & MOTORMODE_ERROR))
                {
                TRACE(("error: motor1 on mtrcnt(%d) faulted", controller.imotorctlr+1));
                Beep();
                fSuccess = false;
                }
            }
...
BOOL CheckMotorsForFault()
    {
    BOOL fSuccess = true;
    LockDaisy();
    for (int imotorctlr = 0; imotorctlr < imotorctlrMax; imotorctlr++)
        {
        CheckMotorForFault(OUT fSuccess, rgmotorctlr[imotorctlr]);
        }
    ReleaseDaisy();
    return fSuccess;
    }
...
void RecordMotorPower(int imotorctlr, int imtr, int powerArg)
    {
...
void ReadControllerEncoders(OUT BOOL& fSuccess, MOTORCONTROLLER& controller)
    {
    fSuccess = controller.fActive;
    if (fSuccess)
        {
...
//
//
#define waitForMotorControllerCommandCycle0()             { wait1Msec(51); }
#define waitForMotorControllerCommandCycle1(controller)   { if (!controller.fLegoController) wait1Msec(51); }    // 26 is too low (sometimes!), 51 seems reliable
...
#define InitializeLegoControllerIfNecessary()                                                           \
    ...
    if (!legoController.fActive)                                                                        \
        ...
        legoController.fActive           = true;                                                        \
                                                                                                        \
        /* ensure that motors stay in position when we stop them */                                     \
        bFloatDuringInactiveMotorPWM     = false;                                                       \
                                                                                                        \
        /* */                                                                                           \
        MOTORMODE modeHelper;                                                                           \
        SetControllerMtrRunMode(modeHelper, legoController, 0, MOTORMODE_RUN_WITH_CONSTANT_SPEED);      \
        }                                                                                               \
    }
...
void InitializeMotorController(IN OUT STICKYFAILURE& fOverallSuccess, MOTORCONTROLLER& controller, I2CLINK link, short jController /*one-based*/)
    ...
    if (fSuccess)
        ...
        waitForMotorControllerCommandCycle1(controller);
        }
...
SVOCTLR rgsvoctlr[isvoctlrMax+1];

#define servoControllerDummy    rgsvoctlr[isvoctlrMax]    // for use with '0' as jvcPaired in InitializeServo
