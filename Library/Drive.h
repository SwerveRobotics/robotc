...
BOOL CALLEDBY(iTaskMain) DriveForwards(float cmDistance, TRIGGERCONDITIONS& trig, int16 power)
// Drive the indicated distance. Return success or failure.
// NB: Thus function can at times WRITE to the trig conditions.
    {
FunctionStart:
    ...
    if (Abs(cmDistance) >= CM_DRIVE_TOLERANCE)
        ...
        // Set things up so that we can trig when we're supposed to
        ...
        ReadLeftRightEncoders(state.encLeftCur, state.encRightCur);
        ...
        for (;;)
            ...
            if (trig.fTriggered)
                {
                TRACE(("drive: trig condition reached"));

                // If we're done, then we're done!
                if (trig.cmAdditional == 0.0)
                    {
                    fSuccess = true;
                    break;
                    }
...
BOOL DriveForwards(float cmDistance)
    {
    InitializeTriggerConditions();
    return DriveForwards(cmDistance, trig, DRIVE_POWER_MEDIUM);
    }
