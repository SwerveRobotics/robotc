...
BOOL DoAutonomous()
    ...
    if (fGoForMiddleDispenser)
        ...
        int cmExtra = (STARTING_SIDE_RIGHT==allianceOption ? 0 : 0);
...
// through in the process.
    ...
    trig.fStopOnTouchPreload = true;
    RET_IF_FAIL(TurnRight(angleSweepMax, TURN_POWER_MEDIUM, trig), "TurnRight");
    ANGLE angleSweepActual = (trig.fTriggerConditionReached ? (1.0 - trig.fractionRemaining) : 1.0) * angleSweepMax;

    // Back up a bit to get the preload arm right over the goal
    const ANGLE angleSweepAdjust  = -2.0;
    InitializeTriggerConditions();
    RET_IF_FAIL(TurnRight(angleSweepAdjust, TURN_POWER_FAST, trig), "TurnLeft");
