...
BOOL CALLEDBY(iTaskMain) TurnRightCore(ANGLE angle, int16 power, TURNFLAG flags, ENCOD dencLeft, ENCOD dencRight, TRIGGERCONDITIONS& trig, float balance=TURN_BALANCE)
    {
    BOOL fSuccess = !trig.fRequireTrigger;

    // Figure out if we need to need to do an encoder-based turn
    // or whether we can use the gyro. In the former case, the caller
    // might have provided the encoder values already, o                              r we might
    ...
    if ((fUseGyro && angle != 0.0) || (dencLeft!=0 || dencRight!=0))
        ...
        ReadLeftRightEncoders(stateLeft.encStart, stateRight.encStart);
        ...
        // If we're using the gyro, then add that to our list of trigger conditions
#if SensorIsDefined(sensnmGyroHorz)
        ...
        if (fUseGyro)
            ...
            // In the below, we need to know where the gyro is (we use sensGyroHorz.deg)
            // so we force a read just to be sure it's present.
            READ_SENSORS_CONTEXT context;
            InitializeReadSensorsContext(context, sensGyroHorz.sensorReadFrequency);
            ...
            // Make sure we trig on that condition
            sensGyroHorz.fTriggerRequested = true;

            ReleaseBlackboard();
            }
#endif
        // Start the monitoring of the sensors for detecting when we should trig
        ArmTriggerConditions();
        ...
        for(;;)
            ...
            if (fUseGyro)
                ...
                {
                ReadLeftRightEncoders(stateLeft.encCur, stateRight.encCur);
            ...
            if (trig.fTriggered)
                ...
                if (!fUseGyro)
                    ...
                    trig.fractionRemaining = leftFraction;
                    TRACE(("turn: trig reached: %1.3f %1.3f", leftFraction, rightFraction));
                    }
                else
                    {
                    #if SensorIsDefined(sensnmGyroHorz)
                    LockBlackboard();
                    trig.fractionRemaining = (float)abs(sensGyroHorz.deg - sensGyroHorz.degTarget) / abs(angle);
                    ReleaseBlackboard();
                    TRACE(("turn: trig reached: %1.3f", trig.fractionRemaining));
                    #endif
                    }
                break;
                }
        ...
        //
        StopRobot();
        DisarmTriggerConditions();
#if SensorIsDefined(sensnmGyroHorz)
        if (fUseGyro && trig.fTriggered)
            {
...
BOOL TurnRight(ANGLE angle, int16 powerLevel, TRIGGERCONDITIONS& trig, float balance=TURN_BALANCE)
    {
    return TurnRightCore(angle, powerLevel, TURNFLAG_DEFAULT, 0, 0, trig, balance);
    }

BOOL TurnRight(ANGLE angleToTurn, int16 powerLevel, float balance)
    {
    InitializeTriggerConditions();
    return TurnRight(angleToTurn, powerLevel, trig, balance);
    }

BOOL TurnRightEncod(int powerLevel, ENCOD dencLeft, ENCOD dencRight, float balance=TURN_BALANCE)
    {
    InitializeTriggerConditions();
    return TurnRightCore(0, powerLevel, (TURNFLAG)(TURNFLAG_USE_ENCOD | TURNFLAG_DEFAULT), dencLeft, dencRight, trig, balance);
    }
...
#if SensorIsDefined(sensnmGyroHorz)
void TurnRightWithGyro(ANGLE angleToTurn)
	{
	ANGLE startAng = sensGyroHorz.deg;
	ANGLE end = startAng + angleToTurn;
	for (;;)
		{
		ANGLE cur = sensGyroHorz.deg;
		ANGLE remaining = end - cur;
		NormalizeAngleVar(remaining);
		...
		if (remaining > 30)
		    ...
		    {
		    int power = (int)((7/3) * remaining + 30);
		    SetMotorPower(motorLeft, power);
		    SetMotorPower(motorRight, -power);
		    }
