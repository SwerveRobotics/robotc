...
typedef enum
    {
    STARTING_SIDE_LEFT  = 0,
    STARTING_SIDE_RIGHT = 1,
    } STARTING_SIDE;

string rgstrProgramFlavor[]  = { "tele op",   "auto"          };
string rgstrTeamColor[]      = { "blue team", "red team"      };
string rgstrAllianceOptionDefault[] = { "left side", "right side"    };

// By using
//
//   #define rgstrAllianceOption yourGlobalArrayOfStrings
//
// before the #include "FTCField.h", you can customize the names of the
// alliance options; the number of alliance options is taken from the
// number of strings in the array.
//
#ifndef rgstrAllianceOption
#define rgstrAllianceOption rgstrAllianceOptionDefault
#endif
...
#ifndef DEFAULT_ALLIANCE_OPTION
#define DEFAULT_ALLIANCE_OPTION 0
#endif
...
int             allianceOption   = DEFAULT_ALLIANCE_OPTION;     // what's the current value of the alliance option
BOOL            fUseProgramDemoMode = false;                    // are we to use our 'demo' mode (if we have one) or not?

//--------------------------------------------------------------------------------------
// Team selection and field side selectionb
//--------------------------------------------------------------------------------------
...
//  (b) the side of the field from which we are starting (left vs right) OR your choice of custom alliance options
//
// The cycle order traversed by the arrow keys is as follows:
//
//   (left/right/etc) # red # tele # blue # (left/right/etc)
//
...
//
#define CycleAllianceOption()                                                                           \
    {                                                                                                   \
    int cAllianceOption = sizeof(rgstrAllianceOption) / sizeof(rgstrAllianceOption[0]);                 \
                                                                                                        \
    allianceOption++;                                                                                   \
    if (allianceOption == cAllianceOption)                                                              \
        allianceOption = 0;                                                                             \
                                                                                                        \
    DisplayFormattedMessageTemporarily1("%s", rgstrAllianceOption[allianceOption]);                     \
                                                                                                        \
    PlayHappyNoWait();                                                                                  \
    for (int i = 0; i <= allianceOption; i++)                                                           \
	    {                                                                                               \
	    int msBeep = 250;                                                                               \
	    wait1Msec(msBeep);                                                                              \
	    Beep(NOTE_A, msBeep); wait1Msec(msBeep);                                                        \
	    }                                                                                               \
    }
