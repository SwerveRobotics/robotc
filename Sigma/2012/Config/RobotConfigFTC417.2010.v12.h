...
void ReadSensors(READ_SENSORS_CONTEXT& context)      // called with the Blackboard lock held
    {
    ReadGyroSensor    (sensGyroHorz,       context);
    ReadEOPDSensor    (sensEopdFront,      context);
    ReadColorSensor   (sensColor,          context);
    ReadEOPDSensor    (sensEopdFrontRight, context);
    ReadEOPDSensor    (sensEopdArm,        context);
    ReadAngleSensor   (sensAngleRotor,     context);
    ReadMagneticSensor(sensMagRotor,       context, sensAngleRotor.deg);
    ReadMagneticSensor(sensMagRotorAux,    context, sensAngleRotor.deg);
    }

void TriggerSensors(TRIGGER_SENSORS_CONTEXT& context)
    {
    TriggerGyroSensor (sensGyroHorz,      context);
    TriggerEopdSensor (sensEopdFront,     context);
    TriggerColorSensor(sensColor,         context);
    TriggerEopdSensor (sensEopdFrontRight,context);
    TriggerEopdSensor (sensEopdArm,       context);
    TriggerAngleSensor(sensAngleRotor,    context);
    TriggerMagneticSensor(sensMagRotor,   context);
    TriggerMagneticSensor(sensMagRotorAux,context);
    }
...
void InitializeServos(IN OUT STICKYFAILURE& fOverallSuccess)
    ...
    /**/
    svoArmElbow     .svposUpper     = 233; /* empirical: *just* avoids hitting the trig */
