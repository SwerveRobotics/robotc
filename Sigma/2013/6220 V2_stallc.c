...
#pragma config(Motor,  mtr_S1_C1_1,     motorBL,       tmotorTetrix, PIDControl, encoder)
...
#include "rdpartyrobotcdr-3.3.1\drivers\lego-ultrasound.h"

#define sonarLeft             msensor_S4_1
#define sonarRight            msensor_S4_2
...
task main()
{
	...
	while (true) // infinite loop
	{
		...
		{
			servo[servoSweeper] = 255; // spin sweeper in
		}
		else if(joy2Btn(7))
		{
		...
		{
			servo[servoSweeper] = 128; // don't spin sweeper
		}

		// line up with flag
		if(joy1Btn(2) && joy1Btn(4)) // two buttons so Tate doesn't accidentally set it off in the middle of tele-op
		{
